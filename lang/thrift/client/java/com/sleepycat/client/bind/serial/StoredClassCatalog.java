...
/**
 * A <code>ClassCatalog</code> that is stored in a <code>SDatabase</code>.
 *
...
public class StoredClassCatalog implements ClassCatalog {
     ...
     *
     * @throws SDatabaseException if an error occurs accessing the database.
     *
     * @throws IllegalArgumentException if the database is not a BTREE database
     * or if it configured to allow duplicates.
     */
    public StoredClassCatalog(SDatabase database)
        throws SDatabaseException, IllegalArgumentException {

        db = database;
        SDatabaseConfig dbConfig = db.getConfig();
        SEnvironmentConfig envConfig = db.getEnvironment().getConfig();

        writeLockMode = (DbCompat.getInitializeLocking(envConfig) ||
                         envConfig.getTransactional()) ? SLockMode.RMW
                                                       : SLockMode.DEFAULT;
        ...
        SDatabaseEntry key = new SDatabaseEntry(LAST_CLASS_ID_KEY);
        SDatabaseEntry data = new SDatabaseEntry();
        if (dbConfig.getReadOnly()) {
            /* Check that the class ID record exists. */
            SOperationStatus status = db.get(null, key, data, null);
             ...
             */
            SOperationStatus status = db.get(null, key, data, null);
            if (status == SOperationStatus.NOTFOUND) {
    ...
    public synchronized ObjectStreamClass getClassFormat(byte[] classID)
        ...
        return getClassFormat(classID, new SDatabaseEntry());
    }

    /**
     * Internal function for getting the class format.  Allows passing the
     * SDatabaseEntry object for the data, so the bytes of the class format can
    ...
    private ObjectStreamClass getClassFormat(byte[] classID,
        ...
        if (classFormat == null) {
            ...
            SDatabaseEntry key = new SDatabaseEntry(keyBytes);

            /* Read the class format. */

            SOperationStatus status = db.get(null, key, data, SLockMode.DEFAULT);
            if (status != SOperationStatus.SUCCESS) {
    ...
    private ClassInfo getClassInfo(ObjectStreamClass classFormat)
        ...
        if (classInfo != null) {
            ...
            if (status != SOperationStatus.SUCCESS) {
                ...
                SDatabaseEntry formatData = new SDatabaseEntry();
    ...
    private ClassInfo putClassInfo(ClassInfo classInfo,
        ...
        try {
            ...
            SDatabaseEntry key = new SDatabaseEntry(LAST_CLASS_ID_KEY);
            SDatabaseEntry data = new SDatabaseEntry();
            SOperationStatus status = cursor.getSearchKey(key, data,
                                                         writeLockMode);
            if (status != SOperationStatus.SUCCESS) {
    ...
    private static byte[] getBytes(SDatabaseEntry dbt) {
        byte[] b = dbt.getData();
        if (b == null) {
            return null;
        }
    ...
    public ClassLoader getClassLoader() {
        try {
            ...
            /*
             * SDatabaseException is declared to be thrown by getEnvironment in
