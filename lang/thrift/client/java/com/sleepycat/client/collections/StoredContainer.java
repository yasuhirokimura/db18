...
public abstract class StoredContainer implements Cloneable {
     ...
     * For example, if <code>SCursorConfig.getReadUncommitted</code> returns
     ...
     */
    final StoredContainer configuredClone(SCursorConfig config) {
     ...
     * including a {@code SDatabaseException} on BDB (C edition).
     */
    ...
    final boolean removeKey(final Object key, final Object[] oldVal) {
        ...
        try {
            cursor = new DataCursor(view, true);
            boolean found = false;
            SOperationStatus status = cursor.getSearchKey(key, null, true);
            while (status == SOperationStatus.SUCCESS) {
                ...
                status = areDuplicatesAllowed() ?
                    cursor.getNextDup(true): SOperationStatus.NOTFOUND;
            }
            closeCursor(cursor);
            commitAutoCommit(doAutoCommit);
            return found;
    ...
    final boolean removeValue(Object value) {
        ...
        try {
            cursor = new DataCursor(view, true);
            SOperationStatus status = cursor.findValue(value, true);
