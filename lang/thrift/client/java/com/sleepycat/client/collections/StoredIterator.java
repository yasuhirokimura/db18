...
public class StoredIterator<E> extends BaseIterator<E>
    ...
    public boolean hasNext() {
        ...
        try {
            if (toNext != 0) {
                SOperationStatus status = move(toNext);
    ...
    public boolean hasPrevious() {
        ...
        try {
            if (toPrevious != 0) {
                SOperationStatus status = move(toPrevious);
                if (status == SOperationStatus.SUCCESS) {
                    toPrevious = 0;
                    toNext = MOVE_NEXT;
                    toCurrent = MOVE_NEXT;
        ...
        try {
            SOperationStatus status =
                cursor.getSearchKey(Integer.valueOf(index),
                                    null, lockForWrite);
            setAndRemoveAllowed = (status == SOperationStatus.SUCCESS);
    ...
    private SOperationStatus move(int direction)
        throws SDatabaseException {
